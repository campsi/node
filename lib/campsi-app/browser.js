...
var checkForModifications = function (ctx, next) {
    if ($('.panel.main').hasClass('modified') && prevState.path !== ctx.path) {
        ...
        ctx.handled = false;
        confirm({
            header: ctx.app.translate('confirm.exitWithChanges.header'),
            message: ctx.app.translate('confirm.exitWithChanges.message'),
            confirm: ctx.app.translate('confirm.exitWithChanges.confirm'),
            cancel: ctx.app.translate('confirm.exitWithChanges.cancel')
        }, function (confirmed) {
            if (confirmed) {
                ctx.pushState();
                next();
