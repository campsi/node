...
var updateLinks = function (ctx, next) {
    ...
    $.each(app.panels, function (id, panel) {
        panel.component.setButtonsHref();
    });
    next();
};
...
var getLandingOffsets = function (ctx, next) {
    if (ctx.path === '/') {
        ctx.campsiApp.panels.welcome.component.component.getOffsets();
    }
    next();
};
$(function () {
    ...
    app.wakeUpPanels(function () {
        ...
        $.each(app.panels, function (id, panel) {
            panel.component.bind('login', function () {
                auth('signup');
            });
        });
        app.routes.forEach(function (route) {
            page(route.path, checkForModifications, layout.loading.show, load(route.resources), layout.next(route.layout), route.onEnterBrowser.bind(route), layout.exec, layout.loading.hide, updateLinks, getLandingOffsets, sendPageView, endRoute);
        });
        //page('*', function (ctx) {
