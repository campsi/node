...
module.exports = function (router, context) {
    ...
    registerLoadHook('projects', function (projects, cb) {
        panels.projects.setValue(projects, cb, false);
    });
    ...
    registerLoadHook('project', function (project, cb) {
        panels.project.setValue(project, cb, false);
    });
    ...
    registerLoadHook('collection', function (collection, cb) {
        ...
        async.parallel([
            function (iterationCallback) {
                panels.collection.setValue(collection, iterationCallback, false);
            },
            function (iterationCallback) {
                panels.designer.setValue(collection, iterationCallback, false);
            },
            function (iterationCallback) {
                panels.entry.component.setOptions(collection, iterationCallback, false);
            }
        ], cb);
    });

    registerLoadHook('entriesAndDrafts', function (entriesAndDrafts, cb) {
        panels.entries.setValue(entriesAndDrafts, cb, false);
    });

    registerLoadHook('entry', function (entry, cb) {
        panels.entry.setValue(entry, cb, false);
    });

    registerLoadHook('draft', function (draft, cb) {
        panels.entry.setValue(draft, cb, false);
    });

    registerLoadHook('components', function (components, cb) {
        panels.components.setValue(components, cb, false);
    });
