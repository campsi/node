...
module.exports = function (router, context) {
    ...
    registerLoadHook('projectGuests', function (projectGuests, cb) {
        var projectUsersValue = panels.projectUsers.value;
        projectUsersValue.guests = projectGuests;
        panels.projectUsers.setSavedValue(projectUsersValue, cb);
    });

    registerLoadHook('projectUsers', function (projectUsers, cb) {
        var projectUsersValue = panels.projectUsers.value;
        projectUsersValue.users = projectUsers;
        panels.projectUsers.setSavedValue(projectUsersValue, cb);
    });
    ...
    router(routes.dashboard.path, routeMatch, load('projects'), function (ctx, next) {
        panels.dashboard.component.widgets.forEach(function (widget) {
            widget.reload();
        });
    ...
    router(routes.entries.path, routeMatch, load('project', 'collection', 'entriesAndDrafts'), function (ctx, next) {
        ctx.isNewEntry = true;
        panels.entry.mountNode.addClass('new');
        panels.entry.mountNode.removeClass('draft');
        panels.entry.setSavedValue({}, function () {
    ...
    router(routes.entry.path, routeMatch, load('project', 'collection', 'entriesAndDrafts', 'entry'), function (ctx, next) {
        if (ctx.isNewEntry === true) {
            return next();
        }
        panels.entry.mountNode.removeClass('new');
