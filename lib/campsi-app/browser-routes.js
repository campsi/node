...
module.exports = function (router, context) {
    ...
    router(routes.collection.path, load('project', 'collection', 'templates', 'components'), function (ctx, next) {
        layout(routes.collection.layout);
        panels.collection.mountNode.find('.big-title input').focus();
        next();
    });
    ...
    router('*', updateLinks, hideLoading, sendPageView);
