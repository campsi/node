...
module.exports = function (router) {
    ...
    router(routes.newProject.path, updateLinks, function () {
        if(window.CAMPSI_USER){
            comp('project').setValue(undefined, function () {
                ...
                comp('project').mountNode.find('.big-title input').focus();
            })
        } else {
            signin();
        }
    ...
    router(routes.newCollection.path, updateLinks, function (ctx) {
    ...
    router(routes.entry.path, updateLinks, function (ctx) {
        comp('entry').load(ctx.params.project, ctx.params.collection, ctx.params.entry, false, function () {
            layout(routes.entry.layout);
        });
    });


    router(routes.draft.path, updateLinks, function (ctx) {
        comp('entry').load(ctx.params.project, ctx.params.collection, ctx.params.draft, true, function () {
            layout(routes.entry.layout);
        });
    });
