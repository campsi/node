...
module.exports = function (app) {
    ...
    var setNext = function (layout) {
        return function (ctx, next) {
            nextLayout = layout;
            async.forEachOf(app.panels, function(panel, id, cb){
                var visible = false;
                if(nextLayout[id] && nextLayout[id].indexOf('prev') === -1 && nextLayout[id].indexOf('next') === -1){
                    visible = true;
                }
                panel.setVisible(visible, cb);
            }, next);
        }
    };
    ...
    var updateLinks = function (path) {
        return $('[href^="/"]').removeClass('active').filter('[href="' + path + '"]').addClass('active');
    };
