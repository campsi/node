(function () {
    ...
    loader(function () {
        ...
        async.forEach($('.panel'), function (el, cb) {
            ...
            panelComponents.fieldProperties.component.bind('fieldPropertiesChange', function (collection) {
                //console.info("field Properties change", collection);
                //console.info('newValue in properties', JSON.stringify(collection.fields[0].name));
                //console.info('actual value in panel', JSON.stringify(panelComponents.collection.component.value.fields[0].name));

                var newValue = deepcopy(panelComponents.collection.component.value);
                newValue.fields = deepcopy(collection.fields);
                panelComponents.collection.component.setValue(newValue, function () {
                    //console.info("collection did change");
                    //console.info('actual value in panel', JSON.stringify(panelComponents.collection.component.value.fields[0].name));
                });
            });
        });
