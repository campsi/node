...
module.exports = Campsi.extend('component', 'array', function ($super) {
    ...
    return {
        ...
        valueDidChange: function (next) {
            var instance = this;
            async.forEachOf(instance.value, function (itemValue, itemIndex, iterationCallback) {
                if (typeof instance.items[itemIndex] === 'undefined') {
                    Campsi.create('array/item', {
                        ...
                        instance.items[itemIndex] = itemComponent;
                        instance.nodes.items.append(itemComponent.render());
                        iterationCallback();
                    });
                } else {
                    instance.items[itemIndex].setValue(itemValue, iterationCallback, false);
                    instance.nodes.items.append(instance.items[itemIndex].render())
                }
            }, function () {
                var i = instance.value.length;
                var itemNodes = instance.nodes.items.find('> .array_item');

                if (i < itemNodes.length) {
                    for (; i < itemNodes.length; i++) {
                        itemNodes.eq(i).remove();
                    }
                }
                next();
            });
        ...
        valueDidChangeOld: function (next) {
