...
var insertAt = require('./insert-at');
    ...
    return {
        ...
        valueDidChange: function (next) {
            ...
            async.forEachSeries(operations, function (operation, iterationCallback) {
                ...
                var $items = instance.nodes.items.find('> .array_item');
                $items.removeClass('keep');
                instance.items.forEach(function (item, index) {
                    item.index = index;
                    itemNode = item.render();
                    itemNode.addClass('keep');
                });
                instance.nodes.items.find('> .array_item:not(.keep)').remove();
                $items.removeClass('keep');
                instance.items.forEach(function (item, index) {
                    itemNode = item.render();
