...
module.exports = Campsi.extend('component', 'form', function ($super) {
    return {
            ...
            async.forEachOf(options.fields, function getFieldTemplate(field, index, cb) {
                Campsi.get(field.type, function (Component) {
                    Component.prototype.getTemplate(field, function onFieldTemplate(fieldTemplate) {
                        ...
                        fieldTemplates[index] = fieldTemplate.replace(/this/g, 'this.' + field.name);
                        cb();
                    });
                });
            }, function allFieldsTemplated() {
                callback('\n{{#if this}}' + fieldTemplates.join('').replace(new RegExp('\n', 'g'), '\n    ') + '\n{{/if}}\n');
            });
        },
        ...
        listenToFieldEvent: function (fieldName) {
            var instance = this;
            // other event forward
            instance.fields[fieldName].unbind('*');
        ...
        hasFields: function () {
            return typeof this.fields === 'undefined' || typeof this.options === 'undefined' || this.options.fields.length === 0;
        },
        errorsDidChange: function (next) {
            ...
            $super.errorsDidChange.call(instance, function () {
                instance.eachField(function (fieldComponent, fieldName) {
                    if (fieldComponent.errors.length > 0) {
                        instance.errors.push({
                            fieldName: fieldName,
                            errors: fieldComponent.errors
                        });
                    }
                });
                next();
        ...
        valueDidChange: function (next) {
            ...
            var instance = this;
            var previousValue = this.getPreviousValue() || this.getDefaultValue();
