...
Campsi.extend('component', 'form/array', function ($super) {
    ...
    return {
        ...
        init: function (next) {
            ...
            $super.init.call(instance, function () {
                ...
                instance.nodes.newItemForm = $('<form class="newItem"></form>');
        ...
        reorderValueFromNodes: function () {
            ...
            instance.nodes.items.find('> .form_array_item').each(function (i, el) {
                for (j = 0; j < l; j++) {
        ...
        valueDidChange: function (next) {
            ...
            //console.info('previousValue');
            //console.dir(previousValue);
            //console.info('instance.value');
            //console.dir(instance.value);
            //console.table(instance.id, "form/array.valueDidChange", previousValue, instance.value, diff);

            async.forEach(diff, function (operation, cb) {
                var itemValue = instance.value[operation.newIndex];

                if (operation.newIndex === -1) {
                    instance.nodes.items.find('> .form_array_item').eq(operation.oldIndex).addClass('remove');
                ...
                instance.nodes.items.find('> .remove').remove();
