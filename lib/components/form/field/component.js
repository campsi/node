...
module.exports = Campsi.extend('component', 'form/field', function ($super) {
    return {
        ...
        renderValue: function () {
            return $('<div>').addClass('form-field').attr('data-name', this.options.name).append($('<div class="label">').text(this.options.label)).append($('<div class="value">').append(this.component.renderValue()));
        },
        ...
        init: function (next) {
            ...
            $super.init.call(this, function () {
                ...
                instance.mountNode.append(instance.nodes.label).append(instance.nodes.control).append(instance.nodes.errors).append(instance.nodes.help);
                next.call(instance);
            });
        },

        eval: function(expression, defaults){
            var instance = this;
            var expr = expression.replace(new RegExp('{([a-zA-Z./_-]+)}', 'g'), function (group, capture) {
                ...
                return typeof obj !== 'undefined' ? '"' + obj.toString() + '"' : '""';
            });
            var booleanValue = defaults || true;
            try{
                var result = math.eval(expr, this.parent.value);
                booleanValue = (result === true || result === 'true');
            } catch (err){
                console.error(err);
                console.error('expr', expr, this.parent.value);
            }
        ...
        update: function () {
            ...
            if (this.options.visible) {
                instance.mountNode.toggle(instance.eval(this.options.visible));
            }
