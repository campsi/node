    ...
    return {
        ...
        valueDidChange: function (next) {
            ...
            if (instance.value._id) {
                ...
                if (instance.value.drafts) {
                    instance.value.drafts.forEach(function (d) {
                        d.__collectionUrl = '/projects/' + instance.project + '/collections/' + instance.collection;
                        if (d._entry) {
                            entries[d._entry].drafts = entries[d._entry].drafts || [];
                            entries[d._entry].drafts.push(d);
                        } else {
                            d.draft = true;
                            drafts.push(d);
                        }
                ...
                this.entryList.setValue(drafts.concat(this.value.entries), next);
        ...
        reload: function (next) {
            ...
            $.getJSON(collectionUrl, function (collection) {
                $.getJSON(collectionUrl + '/drafts', function (drafts) {
