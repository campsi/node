...
module.exports = Campsi.extend('component', 'select/ajax', function () {
    return {
        ...
        reload: function (next) {
            ...
            var instance = this;
            var url = instance.options.useProxy !== false && isBrowser ? config.host + '/utils/ajax-proxy?url=' + this.options.url : this.options.url;
            ...
            if (instance.options.collectionType === 'array') {
                ...
                for (p in ref) {
                    if (ref.hasOwnProperty(p)) {
                        var label = instance.options.paths && instance.options.paths.label ? pathLookup(ref[p], instance.options.paths.label) : ref[p];
                        options.push($('<option>').attr('value', p).text(label));
                    }
                }
            }
            instance.mountNode.empty().append('<option></option>').append(options);
            if (instance.value) {
