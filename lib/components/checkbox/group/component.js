    ...
    return {
        ...
        getDesignerFormOptions: function () {
            ...
            superOptions.fields = superOptions.fields.concat([{
                    ...
                    items: {
                        type: 'form',
                        // todo replace by hash when it's done
                        fields: [
                            {
                                name: 'value',
                                type: 'text',
            ...
            $super.wakeUp.call(this, el, context, function () {
                ...
                async.forEach(checkboxesEls, function (chkEl, cb) {
                    Campsi.wakeUp(chkEl, context, function (component) {
                        instance.checkboxes[$(chkEl).attr('data-checkbox-value').toString()] = component;
                        cb();
                    });
                }, next);
            });
            ...
            for (var name in this.checkboxes) {
                if (this.checkboxes.hasOwnProperty(name)) {
                    ...
                    this.checkboxes[name].bind('change', function () {
                        ...
                        var chkValue = this.mountNode.attr('data-checkbox-value');
        ...
        valueDidChange: function (next) {
            var instance = this;
            async.forEachOf(instance.checkboxes, function eachCheckbox(checkbox, item, cb) {
                checkbox.setValue(instance.value.indexOf(item.toString()) > -1, cb, false);
            }, next);
        },
