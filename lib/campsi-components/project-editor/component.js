...
module.exports = Campsi.extend('component', 'campsi/project-editor', function ($super) {
    return {
        ...
        init: function (next) {
            ...
            $super.init.call(this, function () {
                Campsi.create('form', {
                    ...
                    instance.mountNode.append('<div class="status free">FREE</div>');
                    instance.mountNode.append('<div class="status trial">TRIAL</div>');
                    instance.mountNode.append('<div class="status canceled">CANCELED</div>');
                    instance.mountNode.append('<div class="status ok">OK</div>');
                ...
                fields: [
                    {
                        ...
                        type: 'text',
                        required: true,
                        label: this.context.translate('panels.editProject.fields.title.label'),
                        additionalClasses: ['title', 'horizontal'],
                        placeholder: this.context.translate('panels.editProject.fields.title.placeholder')
                    },
                    {
                        name: 'identifier',
                        type: 'text',
                        label: this.context.translate('panels.editProject.fields.ID.label'),
                        additionalClasses: ['identifier', 'horizontal'],
                        placeholder: this.context.translate('panels.editProject.fields.ID.placeholder')
                    },
                    {
                        name: 'icon',
                        type: 'file/image',
                        label: this.context.translate('panels.editProject.fields.icon'),
                    ...
                    },
                    {
                        ...
                        label: this.context.translate('panels.editProject.fields.url')
                    },
                    {
                        name: 'notes',
                        type: 'text/area',
                        additionalClasses: ['notes', 'horizontal'],
                        label: this.context.translate('panels.editProject.fields.notes.label')
                    },
                    {
                        name: 'collections',
                        type: 'campsi/collection-list',
                        label: this.context.translate('panels.editProject.fields.collections.label')
                    }
                ]
            };
        },
        ...
        valueDidChange: function (next) {
            ...
            this.mountNode.find('.status').hide();

            if (this.value.billing) {
                var subscriptionStatus = this.value.billing.customerDetails.subscriptions.data[0].status;
                if (subscriptionStatus === 'canceled') {
                    this.mountNode.find('.status.canceled').show();
                }
            }
            if (this.value.roles) {
