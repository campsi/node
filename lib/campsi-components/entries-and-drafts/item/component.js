        ...
        valueDidChange: function (next) {
            ...
            instance.context.get('collection', function (collection) {
                instance.FormComponent.prototype.getHtml(instance.value.data, collection, function (html) {
                    instance.mountNode.find('> .main').empty().append(html).append($('<span class="id">').text(instance.value._id.toString()));
                    var resourceType = instance.value.draft === true ? 'draft' : 'entry';
                    var params = {};
                    params[resourceType] = instance.value;
                    instance.mountNode.find('> .main').toggleClass('drag-handle', resourceType === 'entry').attr('href', instance.context.applicationURL(resourceType, params));
                    if (instance.value._entry) {
                        instance.mountNode.find('> .rel').attr('href', instance.context.applicationURL('entry', { entry: instance.value._entry })).html('<i class="fa fa-link"></i> ' + instance.context.translate('panels.entriesAndDrafts.seePublished')).removeClass('hidden');
                    } else if (instance.value.draft && resourceType === 'entry') {
                        instance.mountNode.find('> .rel').attr('href', instance.context.applicationURL('draft', { draft: instance.value.draft })).html('<i class="fa fa-link"></i> ' + instance.context.translate('panels.entriesAndDrafts.seeDraft')).removeClass('hidden');
                    } else {
