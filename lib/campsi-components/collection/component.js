...
module.exports = Campsi.extend('component', 'campsi/collection', function ($super) {
    ...
    return {
        ...
        setFields: function(fields, next){
            this.setValue(extend({}, this.value, {fields: fields}), next);
        },
        ...
        save: function (data) {
            ...
            $.ajax({
                ...
                ctx.get('project', function (currentProject) {
                    ctx.invalidate('project', function () {
                        ctx.set('collection', newValue, function () {
                            page(ctx.applicationURL('collection', {project: currentProject, collection: newValue}));
                        });
                    });
                });
            }).error(function () {
        ...
        delete: function () {
            ...
            if (this.value._id) {
                $.ajax({
                    ...
                    ctx.get('project', function (currentProject) {
                        ctx.invalidate('project', function () {
                            page(ctx.applicationURL('project', {project: currentProject}));
                        });
                    });
                }).error(function () {
