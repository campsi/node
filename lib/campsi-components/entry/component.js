        ...
        init: function (next) {
            ...
            $super.init.call(this, function () {
                Campsi.create('form', {
                    ...
                    instance.mountNode.append('' + '<div class="big-buttons">' + '   <button class="save">' + instance.context.translate('entry.save.button') + '</button> ' + '   <button class="publish">' + instance.context.translate('entry.publish.draft') + '</button>' + '   <button class="remove">' + instance.context.translate('entry.remove.draft') + '</button>' + '</div>');
                    next();
                });
            });
        },
        ...
        delete: function () {
            ...
            $.ajax({
                ...
                instance.setValue({}, function () {
                    ...
                    instance.context.setExpired([
                        'entriesAndDrafts',
                        'entry',
                        'draft'
                    ]);
                    instance.setValue(instance.getDefaultValue(), function () {
                        page(instance.context.applicationURL('collection'));
                    });
                });
            });
        },
        publish: function () {
            ...
            if (isDraft) {
                ...
                if (this.value._entry) {
                    // the draft references an entry
                    url += '/' + this.value._entry;
                    method = 'PUT';
                }
                body._draft = this.value._id;
            } else if (this.value._id) {
                // user is editing a published entry and wants to publish its content
                url += '/' + this.value._id;
                method = 'PUT';
            }
            $.ajax({
